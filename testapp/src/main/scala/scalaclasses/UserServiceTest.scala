...
class UserServiceTest {
  ...
  def UserFunctionsetClientFunctionsIntoContext(xpath: XPath, context: StandardEvaluationContext, document: Document) = {
   /* var classes = xpath.evaluate("//Fonction", document, XPathConstants.NODESET).asInstanceOf[NodeList]
    ...
    context.registerFunction("getAge", Class.forName("com.vermeg.testapp.services.TestService").getDeclaredMethod("getAge", Class.forName("java.lang.String")))
    context.registerFunction("getcurrency", Class.forName("com.vermeg.testapp.services.TestService").getDeclaredMethod("getcurrency", Class.forName("java.lang.String")))
